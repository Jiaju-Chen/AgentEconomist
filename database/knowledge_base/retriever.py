...
class RetrievalResult:
    ...
    def format_markdown(self) -> str:
        """格式化为 Markdown 文本"""
        lines = [f"## 检索结果: {self.query}\n"]
        lines.append(f"共找到 {self.total_found} 条相关结果\n")
        
        for i, result in enumerate(self.results, 1):
            doc = result.document
            lines.append(f"### {i}. {doc.title}")
            lines.append(f"- **期刊**: {doc.journal}")
            lines.append(f"- **发表时间**: {doc.publish_time}")
            lines.append(f"- **相似度**: {result.score:.3f}")
            lines.append(f"- **文档类型**: {doc.doc_type}")
            
            if doc.section_title:
                lines.append(f"- **章节**: {doc.section_title}")
            
            if doc.key_topics:
                lines.append(f"- **关键主题**: {', '.join(doc.key_topics[:5])}")
            ...
            if doc.pdf_link:
                lines.append(f"[查看原文]({doc.pdf_link})\n")
...
class PaperRetriever:
    ...
    def search(
        ...
        Args:
            ...
            doc_type: 文档类型过滤 ("abstract" | "section")
